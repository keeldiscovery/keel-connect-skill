...
make dist                             # build the four packaging trees; L1 does this itself
...
- **`dist/` is generated, never committed** (D1, D7). `make dist` copies and never transforms:
  the only generated bytes in any tree are the two manifests and the version they carry. Never
  hand-edit a built tree -- L1 (`tests/test_dist.py`) compares all four byte for byte, and the
  right place for a change is `packaging/` or the one source copy.
